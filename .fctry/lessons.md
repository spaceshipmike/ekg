...
### 2026-07-26T21:45:00Z | #fctry-core
**Status:** candidate | **Confidence:** 1
**Trigger:** retry-success
**Context:** PR #14 (cargo-dist release workflow) went through six Codex rounds. Round 4 "fixed" write-token exposure by moving `GH_TOKEN` from job-level `env:` to the single `gh release create` step; round 5 correctly rejected that as theater.
**Outcome:** GitHub Actions mints the job token per-job based on the job's `permissions` block — step-level `env:` scoping controls only which process sees the variable, not what earlier steps on the same runner can do (poison PATH, `$GITHUB_ENV`, the filesystem) before the token-bearing step runs. The real boundary is the job: the final workflow runs every third-party binary (cargo-dist installer, cached `dist`) in `contents: read` jobs and reserves one minimal `contents: write` job (`create-release`) that executes only SHA-pinned official actions plus `gh`, with no checkout.
**Lesson:** In GitHub Actions, the security boundary for a write-capable token is the job, never the step. If untrusted/third-party code must run anywhere in a workflow, put it in a read-only job and move the privileged operation to its own job whose steps you fully control. Step-scoped `env:` for secrets is defense-in-depth at best and a false claim of isolation at worst.
