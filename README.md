...
Once the first tagged release ships, `.github/workflows/release.yml` (generated by [cargo-dist](https://github.com/axodotdev/cargo-dist)) will publish prebuilt binaries and a curl installer to GitHub Releases for each pushed `vX.Y.Z` tag, and will update the Homebrew tap formula to point at those prebuilt binaries. Until that first tag lands, the tap formula still builds from source — both routes above build from source today (the Homebrew route needs no Rust toolchain of your own — brew provides it at build time).
