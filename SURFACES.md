...
| Landing-page analytics | `docs/umami.js` (vendored MIT-licensed Umami v2 tracker, same-origin) + the script tag in `docs/index.html` (website id `96bac56f-4b44-4722-ae58-c48ccd4a5523`, `data-host-url=https://umami.h3r3.com`, `data-domains=spaceshipmike.github.io`) | upgrade/re-vendor the tracker, change the collector host or website id, or touch the page's script tags | tracker stays a vendored same-origin copy carrying Umami's MIT notice — never a remote cross-origin script (the page distributes install commands); `data-domains` stays pinned to the production hostname; pageviews visible at umami.h3r3.com after a deploy |
