...
- **`release.yml` is hand-hardened beyond what `dist generate` emits, and must stay that way** — **PROSE**. Tag values (`github.ref_name` / `needs.plan.outputs.tag`) are passed into every job via a quoted `TAG` env var and referenced only as `"$TAG"` in shell — never spliced directly into a `run:` script — to close a shell-injection path a crafted tag name (`v1.2.3$(...)`) would otherwise open. Every `uses:` action reference is pinned to a full commit SHA (matching `ci.yml`'s style). The workflow-level `permissions` default is `contents: read`; exactly one job (`create-release`, which runs no third-party code — only SHA-pinned official actions and `gh`) escalates to `contents: write` via a job-level block. Every job that downloads or executes the third-party cargo-dist binary (`plan`, the build jobs, `host`) runs with a read-only job token, so a compromised dist release never shares a runner or a token with write authority. Homebrew publishing (`publish-homebrew-formula`) has been removed from the generated job graph — see "Open tensions" below. Because these hand edits make the file diverge from a fresh `dist generate`, `dist-workspace.toml`'s `[dist]` section carries `allow-dirty = ["ci"]` so `dist plan`/`dist host`'s "is the workflow up to date" check doesn't fail on every push. Any future `dist generate`/`dist init` re-run must reapply this hardening (including re-removing the Homebrew job and re-scoping permissions) before merging its output.
