...
- **`release.yml` is hand-hardened beyond what `dist generate` emits, and must stay that way** — **PROSE**. Tag values (`github.ref_name` / `needs.plan.outputs.tag`) are passed into every job via a quoted `TAG` env var and referenced only as `"$TAG"` in shell — never spliced directly into a `run:` script — to close a shell-injection path a crafted tag name (`v1.2.3$(...)`) would otherwise open. Every `uses:` action reference is pinned to a full commit SHA (matching `ci.yml`'s style). The workflow-level `permissions` default is `contents: read`; only the `plan` and `host` jobs (the ones that create/upload the GitHub Release) escalate to `contents: write` via a job-level block, so a pull_request-triggered run of `plan` that merely executes `dist plan` never holds write access it doesn't need. Homebrew publishing (`publish-homebrew-formula`) has been removed from the generated job graph — see "Open tensions" below. Because these hand edits make the file diverge from a fresh `dist generate`, `dist-workspace.toml`'s `[dist]` section carries `allow-dirty = ["ci"]` so `dist plan`/`dist host`'s "is the workflow up to date" check doesn't fail on every push. Any future `dist generate`/`dist init` re-run must reapply this hardening (including re-removing the Homebrew job and re-scoping permissions) before merging its output.
